...
void main() {
  ...
  vec3 rampColor = mix(texture2D(uRampMap, rampNightUV), texture2D(uRampMap, rampDayUV), isDay).rgb + vec3(0., 0.2, 0.2);

  vec3 col = csm_DiffuseColor.rgb;
  vec3 darkCol = col * rampColor;

  csm_Emissive = mix(darkCol, col, sdf);
//  csm_Emissive = vec3(sdfSampleRight);
